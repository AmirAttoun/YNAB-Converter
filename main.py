...
CSV_CONFIGS = {
    'Migrosbank': {
        ...
        'delimiter': ';',
        'skip_first_row': True,
        'mapping': {"Datum": "Datum", "Buchungstext": "Buchungstext", "Betrag": "Betrag", "Valuta": "Valuta"},
        'date_conversion': None,
        'type': "Debit"
    },
    'Viseca (One)': {
        'headers': ["TransactionID", "Date", "Merchant", "Amount", "PFMCategoryID", "PFMCategoryName"],
        'delimiter': ',',
        'skip_first_row': True,
        'mapping': {"Date": "Valuta", "Merchant": "Payee", "Amount": "Betrag"},
        'date_conversion': {"source_format": "%Y-%m-%dT%H:%M:%S", "target_format": "%d.%m.%y"},
        'type': "Credit"
    }
...
async def create_upload_file(file_upload: UploadFile, request: Request):
    ...
    if not data:
        ...
        #Read the csv
        filedata = read_and_normalize_csv_data(UPLOAD_DIR / file_upload.filename, bank, CSV_CONFIGS, STANDARD_HEADERS)

        config = CSV_CONFIGS.get(bank)
        account_type = config.get('type')

        return templates.TemplateResponse(
        request=request, name="verify.html", context={"filedata": filedata, "account_type": account_type}
        )
